...
TEST_JARS=$(find $ROOT -type f -name *\.jar | grep -v patchprocess | egrep '/flume-.*-tests\.jar' | sort)
for JAR in $TEST_JARS; do
  echo "-injars $JAR" >> $TEST_OUT_CONF
done

FLUME_JARS=$(find $DIST/target/apache-flume-*-bin/|grep jar | egrep '(lib|tools)/flume.*\.jar' | sort)
for JAR in $FLUME_JARS; do
  echo "-libraryjars $JAR" >> $TEST_OUT_CONF
  echo "-injars $JAR" >> $MAIN_OUT_CONF
done

LIB_JARS=$(find $DIST/target/apache-flume-*-bin/|grep jar | egrep -v '(lib|tools)/flume.*\.jar' | sort)
